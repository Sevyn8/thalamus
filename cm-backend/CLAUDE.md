...
### D-41 — 2d-send uses a password-change ticket delivered by CM-owned SendGrid email (2026-07-19)

**What.** Slice 2d-send (sending the invite to a provisioned INVITED tenant-user) uses a password-change ticket generated via the Auth0 Management API, delivered by a CM-owned email sender (SendGrid). Flow: 2c (unchanged) pre-creates the Auth0 user with `app_metadata` (`tenant_id`, `user_type`, `cm_user_id`), the reliable claim-stamping path; 2d-send generates a password-change ticket (`POST /api/v2/tickets/password-change`) for that pre-created user, CM emails the ticket URL to the invitee via SendGrid, and it sets `invited_at` on the committed row. The invitee sets their credential, logs in (their token carries the `app_metadata`-sourced claims from 2c), and accepts via the committed 2d-accept endpoint.

This SUPERSEDES the Option-C direction (an Auth0 Organization invitation creates the user). Option C was rejected because Auth0 Organization invitation `app_metadata` does NOT reliably transfer to the created user's profile `app_metadata` on acceptance: the Management API documents `app_metadata` / `user_metadata` as invitation payload fields, but the docs do not document transfer to the created user, and multiple Auth0 community reports show invitation `user_metadata` / name not populated on the accepted user without a pre-user-registration hook to copy it. Because the `cortex-cm-claims` Action needs the per-user `cm_user_id` claim (which becomes the REQUIRED `AuthContext.user_id`), and Organization metadata is per-org (cannot supply a per-user `cm_user_id`), the invitation path would leave the first token without the `user_id` claim and break verification. 2c's direct `create_user` + `app_metadata` is the reliable claim source and is therefore retained.

Consistency (per D-39, unchanged): DB-first. Generating the ticket and sending the email are external calls after the row is committed; a failure leaves a still-INVITED row that a retry completes. `invited_at` is the "invite sent" marker (PLATFORM staff actor, AI-TU-08 pair). Email send maps to a typed error, never a raw 500. SendGrid is a new external integration behind a mockable seam (like the `Auth0ManagementClient`); the test suite never sends real email. The SendGrid API key + verified sender ("from" address, default `noreply@sevyn8.com`) are new settings (Secret Manager sourced; permissive at Settings-load, required at send time).

**Why.** This preserves the committed 2c (Step 8.6) reliable claim-stamping path, avoids depending on undocumented / unreliable Auth0 invitation `app_metadata` transfer, and avoids the alternative (a post-login Action resolving `cm_user_id` via a CM lookup endpoint), which adds moving parts and couples login to a CM round-trip. The cost accepted: CM takes on operating email infrastructure (SendGrid). Rejected alternatives: Org invitation + Action-resolves-claims (more architecture, login-time CM coupling); Org invitation + post-accept `app_metadata` patch (ordering problem: the first token lacks the `user_id` claim needed even to reach the accept endpoint); staff-hand-delivered ticket URL (no email infra, but manual and not a real onboarding UX).

**How to apply.** Slice 2d-send builds: (1) a new `create_password_change_ticket` method on the `Auth0ManagementClient` (`POST /api/v2/tickets/password-change`; the 2b/2c methods `create_user` + `update_user_app_metadata` are retained as the claim source per this decision); (2) a CM email sender (SendGrid, behind a mockable seam, offline-tested); (3) a PLATFORM-gated send-invitation staff action that generates the ticket, emails it, and sets `invited_at` on the committed row. New settings: `sendgrid_api_key`, `sendgrid_from_email` (default `noreply@sevyn8.com`), and the Auth0 ticket `result_url` as needed, all permissive at load and required at send time with their own validation. There are no committed Option-C artifacts to clean up (Option C was never built).

**Reconsider if.** Email volume or deliverability needs outgrow SendGrid's free tier (upgrade or switch provider; the mockable seam isolates this), or a future self-serve onboarding flow makes Auth0-native invitation emails preferable despite the `app_metadata`-transfer limitation (which would require the Action-resolves-claims mechanism this decision declined).

**Affects.** Resolves the 2d-send mechanism question left open under D-40 (send side was scheduled, mechanism undecided). Retains 2c (Step 8.6) unchanged. Depends on D-39 (DB-first provisioning), D-40 (accept side), Slice 1 (verification), Slice 2c (`create_user` + `app_metadata` claim source). Introduces CM's first email / delivery infrastructure. FN-AB-40 (email reconciliation) unaffected.

### D-42 — Email-change Auth0 reconciliation is in-request (DB-write, then Auth0, then commit; rollback on Auth0 failure) (2026-07-20)

**What.** Slice 2e makes the EXISTING `PATCH /api/v1/tenant-users/{user_id}` email edit correct by keeping Auth0 in sync in the SAME request; it is not a new action. The shape is in-request, DB-first-ordered: `_repo.update` writes the new email into the open request transaction (pending, not yet committed), then `mgmt.update_user_email` is called as the LAST operation before the handler returns (only when `auth0_sub IS NOT NULL`), then the request transaction commits at dependency teardown. This works because CM's lifecycle (`db/session.py`) spans the handler in one `session.begin()`: commit happens after the handler returns, and any exception that escapes the handler rolls back (`get_tenant_session_dep` does not catch exceptions). So if the Auth0 call raises, the pending email UPDATE is rolled back and both CM and Auth0 stay at the OLD email (a clean atomic fail; the operator retries the whole PATCH).

This is a REFINEMENT of D-39's DB-first model for the in-place-edit case, giving STRONGER atomicity than D-39's tolerated committed-DB / stale-Auth0 window: for a provisioning create there was a naturally-safe intermediate (INVITED with `auth0_sub` NULL), but an in-place email edit on an ACTIVE user has none, so the Auth0 call is ordered inside the transaction with rollback-on-failure instead of as a post-commit follow-on. It is explicitly DISTINCT from the D-39-REJECTED "Auth0-first" shape: Auth0-first fires the Auth0 call before any DB write (risking an Auth0 change with no DB row); here the DB write is ordered FIRST, in-transaction, and a subsequent Auth0 failure undoes it.

- **INVITED (auth0_sub NULL) branch.** Per `ck_tenant_users_auth0_sub_consistency`, INVITED rows have `auth0_sub` NULL, so an email change on an INVITED user is a pure DB write with no Auth0 call (there is no Auth0 user to sync). The handler branches on `auth0_sub is None` and skips the Auth0 call.
- **Residual drift window (documented and accepted).** If the Auth0 call SUCCEEDS but the teardown commit then FAILS (rare: the email UPDATE already flushed; the final COMMIT fails only on connection loss / deferred-constraint / storage error), the result is Auth0-new / CM-old. It is recoverable by an idempotent re-PATCH (same email: the self-excluding uniqueness pre-check is a no-op, the UPDATE re-applies, and the Auth0 update to the same value is idempotent). Accepted as low-probability and self-healing.
- **Verified Auth0 API decisions.** `PATCH /api/v2/users/{id}` with `{"email": <new>}`; INCLUDE `"connection"` (reuse the `auth0_mgmt_db_connection` setting) since the Management API requires `connection` for secondary-identity / database-connection email updates; SET `"email_verified": true` (staff-driven authority per D-12, so no re-verification round trip). Note Auth0's own behavior: a genuine email change invalidates the user's active Auth0 sessions (Auth0's doing, not CM's).

**Why.** The email edit must keep CM and Auth0 in sync, and CM's request lifecycle supports doing it inside the existing PATCH with clean rollback on Auth0 failure, which is both a single non-forgettable operator action and stronger than a separate reconcile action (that would guarantee a drift window until the second step ran, and could be forgotten). Ordering the DB write first (in-transaction) and the Auth0 call last-before-return keeps the DB the source of truth while making an Auth0 failure atomic (rollback), and confines the only real drift to the rare Auth0-success / commit-fail case.

**How to apply.** Slice 2e adds a `update_user_email(user_id, email, connection, email_verified=True)` method to `Auth0ManagementClient` (`PATCH /api/v2/users/{id}`, typed errors -> `Auth0ManagementError`, offline-tested via MockTransport), and extends `patch_tenant_user`: after the repo email write, if `auth0_sub IS NOT NULL`, call `mgmt.update_user_email(user_id=auth0_sub, email=<new>, connection=auth0_mgmt_db_connection, email_verified=True)` as the last operation before returning. `mgmt_client` is reached via `request.app.state.mgmt_client` as in the provision / send handlers. When the management client is unconfigured, the ACTIVE / SUSPENDED email edit surfaces a typed error rather than silently skipping the Auth0 sync (do not commit a CM-only email change for an Auth0-backed user).

**Reconsider if.** The rare Auth0-success / commit-fail drift proves more than theoretical at scale (would motivate a reconciliation sweep or a two-phase approach), or a future requirement needs the email edit to succeed in CM while Auth0 is down (would revert toward D-39's committed-DB / retriable-follow-on for this case, accepting the wider drift window).

**Affects.** Applies D-39's DB-first model to in-place email change (refinement above). SUPERSEDES FN-AB-40 (whose "Auth0-first, then DB" sketch is exactly the ordering D-39 rejected). Depends on D-39 (DB-first), Slice 2c (`auth0_sub` provisioned + the `auth0_mgmt_db_connection` setting), and Slice 1 (`Auth0ManagementClient`). Trade-off accepted: for ACTIVE / SUSPENDED users the email edit now depends on Auth0 availability (Auth0 down -> the edit fails atomically rather than half-applying), which is the correct behavior for keeping CM and Auth0 in sync. Conforms to D-12 (staff-driven authority) and D-24 (identity is `auth0_sub`, unaffected by the email attribute).
...
**Send side mechanism decided (2026-07-19, D-41):** 2d-send generates an Auth0 password-change ticket (`POST /api/v2/tickets/password-change`) for the 2c-pre-created user and delivers the ticket URL by CM-owned SendGrid email, then sets `invited_at` on the committed row (PLATFORM staff action). The Auth0 Organization-invitation path was rejected (invitation `app_metadata` does not reliably transfer to the created user's profile, which would break the required `user_id` claim). See D-41. Build target: Slice 2d-send (introduces CM's first email infrastructure). The earlier "operator decision pending (Org invitation vs password-change ticket)" is now resolved in favor of the ticket + SendGrid.
...
### FN-AB-40 — Email-change Auth0 reconciliation (SUPERSEDED by D-42)

**Superseded (2026-07-20, D-42):** the resolution is D-42, and it overturns this note's "Auth0 first, then DB" sketch (which is exactly the Auth0-first ordering D-39 rejected). D-42's shape is in-request and DB-first-ordered: the repo writes the new email into the open transaction, then `mgmt.update_user_email` runs as the last operation before return (only when `auth0_sub IS NOT NULL`), then the transaction commits; an Auth0 failure rolls the DB write back so both stay at the old email. INVITED (auth0_sub NULL) is a pure DB write. Include `connection` and set `email_verified: true` on the Auth0 PATCH. See D-42. Original forward-note text follows for historical record.
...
**Note on the Dockerfile build-time check and the test that parses it.** The image carries a build-time check that runs on every build (`--no-cache` in `cloudbuild.yaml`, so it cannot be skipped by a cached layer). `ENV ASGI_TARGET` is declared once and read by BOTH the check and the `CMD`, so the string uvicorn resolves at startup is literally the string the build verified. The check resolves that target through `uvicorn.importer.import_from_string` (uvicorn's own resolver, not a reimplementation) under the four environment variables `Settings` has no default for, then asserts a route set registered. `main.py` ends with `app = create_app()` at module level, so importing the module builds the app and therefore needs those four variables; that is why the check supplies them and why the check is worth having at all.

`tests/unit/test_dockerfile_build_check.py` PARSES the Dockerfile and compares both the env list and the route set against the running article, in BOTH DIRECTIONS. The reason is not symmetry, it is that a one-way assertion goes stale silently. A missing environment variable fails the build loudly because `Settings` refuses to construct; a route or a variable that the service no longer wants does not fail anything, so the check keeps passing while asserting less than the comment above it claims. synapse-ui-server's equivalent header records what that cost there: its env list went stale twice, four days apart, in exactly the way its own comment forbade, and its route set had drifted further and more quietly with three commits editing the file in between. A rule enforced by a human remembering is not enforced, and this repository has no CI to notice otherwise. Both directions, always.

Two things about that test which will otherwise be rediscovered:

- **It must disable `Settings`' `env_file`, and that is not test hygiene.** `Settings` declares `env_file = PROJECT_ROOT / ".env"`, and `cm-backend/.env` EXISTS on a developer machine (gitignored, every value populated). Inside the image it does not: `.dockerignore` excludes it, so the build-check's four variables are the only source there. Without switching the file off, every omit-one-variable case reads from disk, does not refuse, and the suite reports health for a Dockerfile list that Cloud Build would reject. The fixture replaces `os.environ` wholesale AND monkeypatches `env_file` to `None` AND clears `get_settings`' `lru_cache` on the way in and out; all three are required.
- **The route assertion is one representative per router, and the RULE is what the test enforces, not the list.** The service registers 73 `(path, method)` pairs across 19 routers, 29 under `/api/v1/tenants` alone, so asserting every pair would be churn on every route added and a set nobody keeps. The realistic failure is a whole router failing to include, which takes all its paths at once. The test therefore fails when a router appears with no representative, so a new router cannot land unasserted; a hand-picked subset is precisely what went wrong on synapse-ui-server, where nobody decided `/alerts` was unimportant, they just never came back. `/api/v1/health` and `/api/v1/me/permissions` are additionally asserted BY NAME, because the per-router rule can be satisfied by a sibling route and those two have consequences the generic rule does not convey: the first is what Cloud Run polls to decide a revision is live, and the second is what cm-frontend's AuthBoundary calls at boot, hard-redirecting to `/auth/logout` on a 401, so an image shipped without it logs every user out rather than degrading.

When adding a route to a NEW router, add one representative to the Dockerfile's `required` set in the same commit. When adding or removing a required setting on `Settings`, update the build-check env list in the same commit. The test tells you either way, before Cloud Build does.
...
- **Step 8.3 (Auth0Client local-JWKS verify) DONE-LOCAL, 2026-07-19.** `Auth0Client` (`auth/auth0.py`) verifies real Auth0 RS256 tokens against Auth0's JWKS (`PyJWKClient`, cached) behind a new `AuthClient` Protocol (`auth/protocol.py`) that both `StubAuthClient` and `Auth0Client` satisfy; `main.py` AUTH0 branch constructs it (pending-Auth0 `NotImplementedError` removed); one new setting `auth0_jwks_url` (derived default); namespace `https://sevyn8.com`; iss/aud reuse `jwt_issuer`/`jwt_audience`. Same `AuthContext` out (D-24). Implements D-07; conforms to D-37 / D-38. 19 offline Auth0 tests + updated lifespan L5; full suite 911 passed; StubAuthClient stays default. Dev deploy + stub-disable + real-token verify remain (see BUILD_PLAN Step 8.3 Remaining).

### Not yet completed

- **Auth0Client dev cutover.** The `Auth0Client` implementation landed at Step 8.3 (DONE-LOCAL, 2026-07-19). What remains: Secret Manager + Cloud Run dev deploy, disabling the stub in dev env vars, and verifying a real Auth0-issued JWT against the dev backend (the latter depends on the Slice 2 claim-stamping, D-38). Production cutover still blocks on this per D-07.
