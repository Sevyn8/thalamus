...
A second assertion covers every platform-only write route (the 4
tenants write endpoints): each must declare ``audience="PLATFORM"``
on its gate. The marker's ``audience`` field is the inspection
target. Without this assertion, a future regression that dropped
``audience="PLATFORM"`` from one of the
...
# Platform-only write routes must declare audience="PLATFORM".
# Method-and-path tuples; matches how FastAPI represents routes.
#
# Includes the two module-access write endpoints (enable / disable),
# which share the same OVERRIDE.GLOBAL gate + audience discipline as
...
_PLATFORM_ONLY_WRITE_ROUTES: frozenset[tuple[str, str]] = frozenset({
    ...
    # role-edit. Gated by ADMIN.ROLES.OVERRIDE.GLOBAL + audience=
    # "PLATFORM". PLATFORM-only by gate-tuple construction (audience-
    # scope coherence: no TENANT role holds .GLOBAL).
    ("PATCH", "/api/v1/roles/{role_id}"),
    # Client-onboarding section writes + wizard-state PATCH +
    ...
    # Client-onboarding document writes. All
...
def test_gate_discipline_platform_only_writes_declare_audience() -> None:
    ...
    Every entry in ``_PLATFORM_ONLY_WRITE_ROUTES``
