...
# not server-side RS256 mint. The script bakes only the JWTs into the
# image at `docker build` time as NEXT_PUBLIC_* build-args. The backend
# base URL is NO LONGER baked: it is a runtime env (API_BASE_URL) read
# via /api/config (runtime-config decoupling). Nothing auth-related
# flows through terraform's runtime_env.
#
