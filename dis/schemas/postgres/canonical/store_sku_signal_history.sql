...
'Append-only daily history of computed signals (derived attributes) per SKU per store per day. Written by the daily compute job; never updated. Plain (non-partitioned) for beta; Slice 21 re-partitions by as_of_date for BQ archive + eviction. Tenant-isolated via RLS. Current values of these signals also live on store_sku_current_position; this table preserves daily history for incremental compute and backtesting.';
