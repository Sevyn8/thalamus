-- ============================================================================
-- DIS staging schema: store_sku_sale_events
--
-- Shadow mirror of canonical.store_sku_sale_events. The streaming consumer
...
-- canonical when investigating across schemas. FK targets unchanged: still
-- point to identity_mirror and config.source_mappings.
--
...
-- 4. staging.tax_treatment_enum exists (created by the
...
--   - function: uuidv7()
--   - type:     staging.tax_treatment_enum
-- ============================================================================
...
CREATE TABLE staging.store_sku_sale_events (
    ...
    store_sku_current_position_id  UUID                             NULL,
        -- Soft cross-reference to staging.store_sku_current_position.id at
...
COMMENT ON TABLE staging.store_sku_sale_events IS
'Shadow mirror of canonical.store_sku_sale_events. Written by the streaming consumer when the active mapping has status=STAGED. Identical shape and constraints to canonical: plain (non-partitioned) for beta; Slice 21 re-partitions by event_date. Tenant-isolated via RLS. Lets the operator inspect realistic event output before promoting the mapping to ACTIVE.';

COMMENT ON COLUMN staging.store_sku_sale_events.id IS
'Surrogate identifier. UUIDv7. Primary key.';

COMMENT ON COLUMN staging.store_sku_sale_events.event_date IS
'DATE derived from source_sale_timestamp::date at UTC. CHECK constraint enforces the derivation. Slice 21''s re-partition key.';

COMMENT ON COLUMN staging.store_sku_sale_events.tenant_id IS
'Tenant owning this row. FK to identity_mirror.tenants. RLS scopes every read and write by this column.';

COMMENT ON COLUMN staging.store_sku_sale_events.store_id IS
'Store where the sale occurred. FK to identity_mirror.stores.';

COMMENT ON COLUMN staging.store_sku_sale_events.sku_id IS
'SKU identifier as received from the source. Same trimming/casing normalization rules as in store_sku_current_position.';

COMMENT ON COLUMN staging.store_sku_sale_events.event_subtype IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.source_sale_timestamp IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.transaction_id IS
'Source POS basket / transaction identifier. Used to reconstruct a basket by grouping rows with the same transaction_id.';

COMMENT ON COLUMN staging.store_sku_sale_events.line_item_seq IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.quantity IS
'Units sold on this line. Positive for SALE; negative for RETURN and VOID. Enforced by ck_st_ssse_return_void_quantity_sign.';

COMMENT ON COLUMN staging.store_sku_sale_events.unit_retail_price IS
'List / regular / everyday price per unit before any discount. The "shelf tag" price at the time of sale.';

COMMENT ON COLUMN staging.store_sku_sale_events.unit_sale_price IS
'Actual price charged per unit after discount. unit_sale_price <= unit_retail_price (enforced).';

COMMENT ON COLUMN staging.store_sku_sale_events.discount_amount IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.sale_channel IS
'POS, OMNI, ECOM, MARKETPLACE. NULL when source does not distinguish.';

COMMENT ON COLUMN staging.store_sku_sale_events.store_sku_current_position_id IS
'Soft cross-reference to staging.store_sku_current_position.id at write time. Not a FK: lifecycle independence (sale events outlive current_position rows for delisted SKUs) and bootstrap (a new SKU''s first sale may be written before current_position exists). NULL when not available.';

COMMENT ON COLUMN staging.store_sku_sale_events.related_sale_event_id IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.mapping_version_id IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.trace_id IS
'End-to-end trace identifier for the chunk that produced this row. Joins to BigQuery audit_events.';

COMMENT ON COLUMN staging.store_sku_sale_events.dis_channel IS
'Ingress channel that delivered the data (csv_upload, api, csv_erp, reverse_api).';

COMMENT ON COLUMN staging.store_sku_sale_events.last_updated_at IS
...
COMMENT ON COLUMN staging.store_sku_sale_events.ingest_metadata IS
