-- ============================================================================
-- DIS staging schema: store_sku_change_events
--
...
--
-- Same partitioning posture as canonical (plain for beta; re-partitioning
-- by event_date is planned). Same atomic dual-write pattern (paired with
...
COMMENT ON COLUMN staging.store_sku_change_events.event_category IS
...
COMMENT ON COLUMN staging.store_sku_change_events.event_subtype IS
'Subtype within event_category. Open vocabulary; documented in table header; validated by Pandera in the streaming consumer.';

COMMENT ON COLUMN staging.store_sku_change_events.source_event_timestamp IS
...
COMMENT ON COLUMN staging.store_sku_change_events.effective_from IS
...
COMMENT ON COLUMN staging.store_sku_change_events.effective_until IS
'When the change ends. NULL means open-ended. Populated only for events with explicit end times (some price promotions).';

COMMENT ON COLUMN staging.store_sku_change_events.attribute_name IS
...
COMMENT ON COLUMN staging.store_sku_change_events.numeric_value_after IS
'Typed shortcut for numeric attributes. Same population rules as numeric_value_before.';

COMMENT ON COLUMN staging.store_sku_change_events.numeric_change IS
...
COMMENT ON COLUMN staging.store_sku_change_events.reason_code IS
'Source''s reason code for the change (e.g., PO_RECEIPT, CYCLE_COUNT, DAMAGE_OUT). Free-text.';

COMMENT ON COLUMN staging.store_sku_change_events.reason_note IS
'Free-text reason or note from source. Up to 256 chars.';

COMMENT ON COLUMN staging.store_sku_change_events.change_context IS
...
COMMENT ON COLUMN staging.store_sku_change_events.mapping_version_id IS
...
COMMENT ON COLUMN staging.store_sku_change_events.trace_id IS
'End-to-end trace identifier for the chunk that produced this row. Joins to BigQuery audit_events.';

COMMENT ON COLUMN staging.store_sku_change_events.dis_channel IS
'Ingress channel that delivered the data (csv_upload, api, csv_erp, reverse_api).';

COMMENT ON COLUMN staging.store_sku_change_events.last_updated_at IS
...
COMMENT ON COLUMN staging.store_sku_change_events.ingest_metadata IS
