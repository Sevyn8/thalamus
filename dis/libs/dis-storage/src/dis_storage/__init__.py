"""dis-storage — the only sanctioned GCS access path in DIS.

- :func:`build_object_path` — the frozen canonical object-path scheme (UUID tenant
  segment). The caller supplies ``trace_id``; the lib never mints it.
...
- :func:`split_object_uri` — split a contract ``gcs_uri`` (``gs://bucket/key``) into
  ``(bucket, object_key)`` for the two callables above; consumers must not
  hand-split URIs.
...
- :func:`generate_upload_url` — V4 signed PUT URL issuance. Deterministic and offline
  (signing needs a signer credential, not a network round-trip). Currently unused in
  production; the receiver upload path uses synchronous stream-through instead.

Issuance correctness offline is a *well-formed* URL, not proof that real GCS accepts
the signature; that remains unverified until a real caller exercises it against real
GCS. Tests sign with a throwaway test credential only.
"""
