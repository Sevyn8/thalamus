...
def test_importing_dis_validation_loads_no_io_or_sibling_modules() -> None:
    probe_lines = ["import sys", "import dis_validation"]
    probe_lines += [
        f"assert {name!r} not in sys.modules, 'dis_validation must not pull in {name}'" for name in FORBIDDEN
    ]
