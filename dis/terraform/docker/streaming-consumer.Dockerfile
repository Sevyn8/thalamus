# streaming-consumer
#
# A long-running Pub/Sub PULL consumer: it subscribes to ingress.ready, runs the
# mapping/normalize/validate pipeline and dual-writes canonical rows (hot + event
# tables) plus quarantine, and in Cloud Run Service mode (RUN_HEALTH_SERVER=true)
# serves a readiness /healthz on the Cloud-Run-injected $PORT alongside the loop
# (slice 40a). Entrypoint:
...
RUN uv sync --frozen --no-dev --package streaming-consumer

ENV PATH="/app/.venv/bin:$PATH"

# Non-root (the consumer writes no local filesystem at runtime).
...
CMD ["python", "-m", "streaming_consumer.main"]
