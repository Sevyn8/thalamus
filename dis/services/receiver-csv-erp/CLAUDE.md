# services/receiver-csv-erp — Claude Code Context

Loaded when Claude Code works in `services/receiver-csv-erp/`. Service-specific rules; root `CLAUDE.md` applies first.

## What this service is

Per-tenant ERP CSV POST endpoint. Accepts CSV pushes from tenant ERP systems on a configured schedule.
...
- Per-tenant rate limit (token bucket) before signed-URL issue.
