...
**DB-pull mode** only: a finite, run-to-completion process that reads
...
Two connections: it **reads** Customer Master (``CM_DB_URL``; read-only role under
...
There is no Pub/Sub consumer mode (``identity.changed``) here. Audit is **log-only**
(no ``audit.events`` rows; no ``dis-audit`` dep).
"""
