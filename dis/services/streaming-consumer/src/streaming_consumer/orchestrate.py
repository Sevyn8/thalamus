...
1. fetch            — cross-check + bronze read + GCS download
2. mapping load     — per-lookup ACTIVE mapping; routing (sale-vs-change)
3. store read       — sale path only: tax_treatment denormalization source
4. pre-validation   — source-shape suite; failure → disposition
...
7. dual-write       — mapping_version_id stamp + atomic hot+event transaction
8. duplicate audit  — ROW-scoped events for dedup-key hits (read-time-dedup posture)

**Failure disposition (audit-and-nack, with a quarantine carve-out):**
...
remains the recoverable source), never partially written (validation precedes
the write; write failures roll the batch back). The governing principle: a
failure waiting for a dependency to arrive (catalogue onboarding → the
...
any IDs it does mint (event-row ids) come from dis-core ``ids``.
"""
...
class ConsumeOutcome:
    ...
    queue — that ack IS the storm fix); every other disposition nacks.
    """
...
# The gate-summary failure codes per validating stage (stable failure vocabulary).
...
# The storm stopper: the NARROW allowlist of exception-path failures KNOWN to
...
# exists):
#   - FailureCode.HOT_POSITION_MISSING — the event rows are already
...
def _quarantinable(exc: Exception, code: FailureCode, ctx: _FlowContext) -> bool:
    """Is this exception-path failure on the quarantine allowlist AND fully describable?
    ...
    - the known-columns guard: ``dis_channel`` only exists post-fetch (the bronze
      ...
      held, so it stays on the nack path;
...
class _FlowContext:
    ...
    ``failure_message`` (the same seam the quarantine records consume:
    ``dis_channel``/``row_count`` exist exactly so a held chunk satisfies the
    quarantine table's NOT NULLs, and the allowlist guard keys on
    ``dis_channel is None`` to refuse pre-fetch holds). ``lap()`` is the
    ...
    def note_bronze(self, bronze: BronzeMeta) -> None:
        ...
        the storm class quarantine exists to hold).
        """
...
class ConsumerPipeline:
    """One consumer process's wired dependencies (caller-owned)."""
    ...
    async def process(self, event: IngressReadyEvent) -> ConsumeOutcome:
        ...
        either holds the chunk in quarantine and returns the ``quarantined``
        disposition the subscriber ACKS (the narrow deterministic allowlist: the
        storm fix), or re-raises so the subscriber nacks (audit-and-nack for
        everything else). The ordering on the quarantine path is load-bearing:
        quarantine-write-loud → QUARANTINED audit-emit-forget → ack; a failed
        HOLD falls back to the nack (never ack-and-lose), a failed AUDIT emit
        never blocks the ack of a successfully-held chunk. The FAILURE audit
        carries every correlation id known at the point of failure
        (``_FlowContext``) and a stable ``FailureCode`` — never a bare exception
        class name.
        """
        ...
        try:
            ...
            try:
                ...
                # the subscriber nacks (the Pub/Sub dead-letter policy backstops
                # the loop).
                _log.bind(
    ...
    async def _process(self, event: IngressReadyEvent, ctx: _FlowContext) -> ConsumeOutcome:
        ...
        # 0. Redelivery detection: best-effort audit readback so a
        ...
        # 1. Fetch (intake + bronze + GCS; the chunk arrives tokenized).
        #    ctx learns the bronze identity MID-stage via note_bronze, so a
        #    download/parse failure still classifies as post-fetch (the
        #    quarantine known-columns guard).
        ...
        # 2. Mapping load (per-lookup side-input) + routing.
        ...
        await self.audit.emit(
            ...
            event_data={
                ...
                # The template the lookup keyed on.
                "template_id": str(event.template_id),
            },
        )

        # 3. Sale + catalogue paths: the store row's enrichment facts (a data read,
        #    not identity validation — the composite FK is the enforcement; no
        #    Identity Service call exists). One widened read: tax_treatment feeds
        #    the event-path injection AND the current-position enrichment;
        #    currency feeds the current-position enrichment.
        ...
        # 4. Pre-mapping (source-shape) gate — semantic validation lives here.
        ...
        # 5b. Enrichment: the lib's value wins over the mapping for its
        ...
        #     the current-position target — the event path is untouched.
        ...
        # 7. mapping_version_id stamp + the write. Two SIBLING paths: the
        #    catalogue (snapshot) path writes the hot table directly via the
        #    reused complete-path upsert (no event table, no dedup); the event
        #    path is the atomic dual-write.
        if loaded.target_model is StoreSkuCurrentPosition:
        ...
        # 8. Duplicate detail: one ROW-scoped event per dedup-key hit.
    ...
    async def _emit_gate_failure(
        ...
        *,
        ...
        """One INGRESS_EVENT FAILURE summary + ROW-scoped events.

        Stable vocabulary: the summary carries the per-gate
        ...
        await self.audit.emit(
            ...
            failure_message=f"{len(failures)} failure(s); chunk nacked",
        )
        for f in failures:
            await self.audit.emit(
                stage=stage,
                ...
                # NOT read — quarantine keeps the value, audit stays value-free
                # by construction.
    ...
    async def _quarantine_gate_failure(
        ...
        *,
        ...
        identically), so they are storm-class too. Routing:
        ...
        A failed HOLD logs and returns ``fallback`` (the ``failed_*``
        ...
        await self._emit_quarantined(
            ...
            table=table,
            held=held,
            message=f"{len(failures)} gate failure(s) held; chunk acked",
        )
        return ConsumeOutcome(disposition="quarantined")
    ...
    async def _emit_quarantined(
        ...
        *,
        ...
        """The QUARANTINED disposition record for the reserved stage, carrying
        the failure-audit shape.
        ...
        every audit emit: a failed emit never blocks the ack of a
    ...
    async def _seen_before(self, tenant_id: UUID, trace_id: UUID) -> bool:
        """Best-effort redelivery detection for the RETRIED outcome.
        ...
        as a first delivery (the duplicate-shaped result is tolerated).

        Where the subscription carries a dead-letter policy (the deployed
        subscriptions do), Pub/Sub populates ``delivery_attempt`` on the pull
        response — a transport-level signal that could replace this readback.
        """
        try:
