"""The three-tier swallow on the ingress.ready pull loop (L6).

The loop still survives everything (a dead consumer is worse than a noisy one), so
...
deadline (measured: 1.06s at timeout=2, 4.05s at timeout=5, 9.12s at timeout=10),
so it never raises DeadlineExceeded at all. The storm is real-Pub/Sub-only.
"""
...
async def test_non_transient_error_logs_bug_at_error(
    ...
    is the failure mode this tier exists to prevent."""
    subscriber = _subscriber(monkeypatch, raises=exc)
    caplog.set_level("DEBUG")
