...
**Run state is derived from the AUDIT TRAIL, not from bronze.** Bronze's
``processing_status`` is an ingress-only signal that the pipeline never advances past
RECEIVED/PUBLISHED; the completion truth lives in ``audit.events``. The displayed ``status``
is the VERDICT derived from the run's terminal audit event.

THE crosswalk: there is ONE terminal-marking
...
posture). The quarantine case keys on the PAIR, never on
...
Counts: three INDEPENDENT recorded numbers — ``input_row_count`` (bronze total),
...
accepted/needs-review/rejected three-way split — the distinction is not recorded.
"""
...
# stalled — the two are not distinguished). The three terminal verdicts derive
...
# to pick the path-aware accepted figure.
...
# ``repos/runs.py`` — they are generated from this tuple, so they cannot drift.
...
def verdict_of(stage: str, outcome: str) -> StatusWire:
    ...
    (the no-silent-fallback posture). The repo only ever hands this a pair that the SQL
...
class RunRow(BaseModel):
    ...
    (store absent on the run, no source-registry row, older runs predate templates),
    never an error. ``auth_principal`` / ``client_ip`` / ``user_agent`` / ``gcs_uri``
    are deliberately ABSENT (caller-context PII + payload location; never on the tenant wire).
    """

    id: str
    trace_id: str
    tenant_id: str  # the owning tenant (bronze.tenant_id, NOT NULL) — fleet attribution
    tenant_name: str | None  # identity_mirror.tenants.name; null when tenant unmirrored
    ...
    template_id: str | None  # bronze replay-lineage template; null on runs that predate it
    ...
    # The audit-derived verdict. Wire key stays ``status`` for contract stability;
    # its VALUE is the verdict, not bronze.processing_status.
    status: StatusWire
    mapping_version: int | None  # from the terminal audit event's mapping_version_id
    seen_before: bool  # a recorded duplicate outcome exists for this run
    source_payload_id: str | None  # the File / event ref (upload_session_id)
    file_name: str | None  # the uploaded file's original name; null on runs that predate it
    # The three INDEPENDENT counts. The response never asserts accepted + quarantined ==
...
class RunListResponse(BaseModel):
    ...
    runs remain. The envelope augments the bare ``{items: [...]}`` house
