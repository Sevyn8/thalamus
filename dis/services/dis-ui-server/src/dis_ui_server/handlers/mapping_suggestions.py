...
handler has no LLM error path of its own.
...
async def post_mapping_suggestions(
    ...
    Type-aware: with a valid ``template_type`` the suggester scores against THAT type's
    ...
    # elapsed_ms: wall-clock of the whole generation — the model call on the llm
