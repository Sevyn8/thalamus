...
import csv_ingest_worker.main as main_module
from csv_ingest_worker.health import Heartbeat, make_healthz_app
from csv_ingest_worker.main import _run
...
def _wire_fakes(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(main_module, "PubsubPublisher", lambda **kw: object())
    monkeypatch.setattr(main_module, "WorkerAudit", lambda writer: object())
    monkeypatch.setattr(main_module, "select_writer", lambda backend, engine: object())
    monkeypatch.setattr(main_module, "IngestPipeline", lambda **kw: object())
