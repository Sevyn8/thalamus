"""Readiness healthz for the pull-loop worker (slice 40a, Cloud Run Service mode).
...
dependency in the worker): ``GET /healthz`` answers 200 while the heartbeat is
fresh and 503 once it goes stale — READINESS, not liveness, so a dead loop behind
a living HTTP server reports unhealthy and Cloud Run restarts the worker (the
