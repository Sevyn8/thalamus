...
async def test_selling_positions_are_a_subset_of_the_universe(
    ...
    assert not orphans, (
        ...
        "SUSPECT TIMING SKEW FIRST, ahead of data. current_state and last_sale_at are fetched in "
        "SEPARATE transactions with no shared snapshot (resolve_declaration documents why one "
        "transaction cannot close that at READ COMMITTED), so a SKU whose FIRST SALE arrives "
        "between the two fetches makes last_sale_at return a position current_state did not — "
        "which looks exactly like this and is pure timing. Re-run: skew does not reproduce, a "
        "real violation does.\n"
        "IF IT REPRODUCES it is a data claim, not a broken constraint. Sale events carry no FK "
        "to the hot table by design (lifecycle independence for delisted SKUs; bootstrap before "
        "a position exists), and those two documented reasons are the two ways it can "
        "legitimately break — so the finding is that dead_stock needs a wider universe source, "
        "not that something should be constrained"
    )
